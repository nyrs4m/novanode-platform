...
export default function DashboardHome({
  ...
  const [activeOrdersCount, setActiveOrdersCount] = useState(initialActiveOrders)
  const [activeTablesCount, setActiveTablesCount] = useState(initialActiveTables)
  ...
  // ── Realtime: keep dashboard counts in sync with KDS ─────────────────
  //
  // We listen to the same three tables the KDS listens to, scoped to this
  // restaurant. On each change we re-derive the counts from a lightweight
  // server refetch rather than trying to track local state diffs (which
  // would require mirroring all the KDS logic here).
  //
  // Pattern: realtime event → trigger refetch of just the count queries.
  // This avoids full page reload while keeping counts accurate.
  //
  useEffect(() => {
